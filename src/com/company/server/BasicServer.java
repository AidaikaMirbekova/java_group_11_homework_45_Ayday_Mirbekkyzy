...
//        server.createContext("/user", this::handleUserIncomingServerRequests);
...
//        registerGet("/user", exchange -> sendFile(exchange, makeFilePath("user.ftl"), ContentType.TEXT_HTML));
//        registerGet("/sample", exchange -> sendFile(exchange, makeFilePath("index.html"), ContentType.TEXT_HTML));


        // эти обрабатывают запросы с указанными расширениями
        registerFileHandler(".css", ContentType.TEXT_CSS);
        registerFileHandler(".html", ContentType.TEXT_HTML);
        registerFileHandler(".ftl", ContentType.TEXT_HTML);
    ...
    public void handleUserIncomingServerRequests(HttpExchange exchange) {
        var route = getRoutes().getOrDefault(makeKey(exchange), this::respond404);
        route.handle(exchange);
    }

    private void handleBooksIncomingServerRequests(HttpExchange exchange) {
        var route = getRoutes().getOrDefault(makeKey(exchange), this::respond404);
        route.handle(exchange);
    }
